...
bool Prog(istream &in, int &line)
{
	...
	if (t != PROGRAM)
	{
		ParseError(line, "Missing PROGRAM Keyword"); // MIGHT NOT NEED
		return false;
	}
	t = Parser::GetNextToken(in, line);
	if (t != IDENT)
	{
		ParseError(line, "Missing Program Name"); // MIGHT NEED TO CHANGE
		return false;
	}
	if (!CompStmt(in, line))
	{
...
bool Stmt(istream &in, int &line)
{
	LexItem t = Parser::GetNextToken(in, line);
	Parser::PushBackToken(t);
	// COMMENT HEREEEEEE COULD BE VERY USEFULL!!!
